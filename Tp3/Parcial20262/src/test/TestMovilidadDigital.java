...
class TestMovilidadDigital {
    ...
    @Test
    void testCalcularRetencionIIBB() {
    	List<Contribuyente> contribuyentes = new ArrayList<>(); 
    	
    	//Cargo los conductores
    	contribuyentes.add(conductor1);
    	contribuyentes.add(conductor2);
    	contribuyentes.add(conductor3);
    	
    	contribuyentes.addAll(conductor1.getViajes());
    	contribuyentes.addAll(conductor2.getViajes());
    	contribuyentes.addAll(conductor3.getViajes());
    	
    	
    	double total = 0;
    	for(Contribuyente c: contribuyentes) {
    		total += c.calcularRetencionIIBB();
    	}
    
    	assertEquals(3217.50, total);
    }
}